"""
电力盗窃检测 (ETD) 的条件扩散模型 (Conditional Diffusion Model) 架构。
结合了特征级线性调制 (FiLM) 与掩码感知 (Mask-aware) 机制，用于时间序列的流形学习与异常打分。
"""
import math
...
# ============================================================
# 1. 基础网络组件 (Utility Blocks)
# ============================================================
class SinusoidalPositionEmbeddings(nn.Module):
    """
    正弦位置编码器。将一维时间步 (Timestep) 映射为高维连续的正弦/余弦嵌入向量。
    """
...
class FiLM1D(nn.Module):
    """
    特征级线性调制 (Feature-wise Linear Modulation) 模块。
    通过条件向量 (cond) 对输入特征进行缩放 (Scale) 与平移 (Shift)。
    """
...
class ResBlock1D(nn.Module):
    """
    包含深度可分离卷积 (Depthwise Convolution)、组归一化与 FiLM 调制的残差块。
    """
...
# ============================================================
# 2. 核心网络 (Conditional U-Net)
# ============================================================
class ConditionalUNet1D(nn.Module):
    """
    条件 1D U-Net 架构，支持通过物理掩码约束提取高维时间序列特征。
    """
    def __init__(
        self,
        in_channels: int = 3,
...
# ============================================================
# 3. 扩散模型生成控制 (Diffusion Wrapper)
# ============================================================
class GaussianDiffusion1D(nn.Module):
    """
    一维高斯扩散包裹器。负责调度正向加噪进程与去噪重构推理。
    包含基于掩码的去噪损失计算机制与 DDIM 快速采样器。
    """
    def __init__(
        self,
        model: nn.Module,
        ...
        # 线性扩散方差调度策略 (Linear Noise Schedule)
    ...
    def compute_anomaly_score(
        ...
        noise_level: int = 150,
        ddim_steps: int = 10,
        k_min: float = 0.05,  # 遇到极锐利尖峰 (高频) 时，最少取 5%
        k_max: float = 0.40   # 遇到平缓误差 (低频) 时，最多取 40%
    ) -> torch.Tensor:
        """
        基于傅里叶频谱分析的自适应异常得分计算 (Frequency-Aware Adaptive Scoring)。
        动态感知重构误差的高频能量占比 (HFER)，为每个样本自适应调节 Top-K 截断比例。
        """
        x_log = self._ensure_channel(x_log)
        mask = self._ensure_channel(mask).float()
        ...
        # 2. 执行快速傅里叶变换 (FFT) 转换到频域
        fft_out = torch.fft.rfft(mse_flat, dim=-1)
        power_spectrum = torch.abs(fft_out) ** 2
        
        # 3. 计算高频能量占比 (High-Frequency Energy Ratio)
        mid_freq_idx = power_spectrum.shape[-1] // 2
        total_energy = power_spectrum.sum(dim=-1).clamp(min=1e-9)
        high_freq_energy = power_spectrum[:, mid_freq_idx:].sum(dim=-1)
        hfer = high_freq_energy / total_energy
        
        # 4. 将 HFER 线性映射到动态 K 比例 [k_min, k_max]
        # HFER 越大 -> 窃电越突发 -> K 越小; HFER 越小 -> 长线窃电 -> K 越大
        hfer_normalized = (hfer / 0.5).clamp(min=0.0, max=1.0)
        dynamic_k_ratio = k_max - hfer_normalized * (k_max - k_min)
        
        # 5. 用户专属 Top-K 截断与得分计算
        score_list = []
        ...
        for i in range(B):
            ...
            user_k_ratio = dynamic_k_ratio[i].item()
            k = max(int(len(valid_mse) * user_k_ratio), 1)
